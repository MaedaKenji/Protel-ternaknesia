...
class DataSapiPage extends StatelessWidget {
  ...
  Future<void> fetchData() async {
    final url = Uri.parse(
        '${dotenv.env['BASE_URL']}:${dotenv.env['PORT']}/api/cows/$id');
    final response = await http.get(url);

    if (response.statusCode == 200) {
      final data = json.decode(response.body);

      // Parse and assign data to the respective lists
      beratBadan = List<double>.from(data['beratBadan'] ?? []);
      susu = List<double>.from(data['susu'] ?? []);
      pakanHijau = List<double>.from(data['pakanHijau'] ?? []);
      pakanSentrat = List<double>.from(data['pakanSentrat'] ?? []);
    } else {
